# -*- coding: utf-8 -*-
"""
Created on Thu Apr  4 12:56:14 2024
...
from Metrics import levenshtein_distance, pairwise_difference
from Metrics import rhythm_grid, progression_grid, fingerprint



class Melody_Player(Player):
    
    """ VARIABLES """
    
    ID = "Melody"
    attention = ['Drums', 'Chords']
    
    libraries = {
        'progressions': "Patterns/Chords/progressions.txt",
        'rhythms':      "Patterns/Melody/rhythms.txt"
        }
    
    general_preference = (2, 2, 3, 3, 5)
    ...
    def random_sequence(self):
        ...
        # Assure the rhythm sequence contains more notes than the progression
    ...
    """
    def resolution(self, output, sequence):
        
        # Before the first 75% of a sequence you can't reasonably say much about the resolution
        if sum([length for _, length in output]) < 12:
            return 0
        
        # Get the fingerprint of the first and last set of output notes
        f1, l1 = fingerprint(output[0][0]), fingerprint(output[-1][0])
        
        # Get the fingerprint of the first and last note of the sequence
        f2, l2 = self.scale.note(sequence[0][0]), self.scale.note(sequence[0][-1])
        f2, l2 = fingerprint(f2), fingerprint(l2)
        
        # If the first note is in the first chord and the last note is in the last chord
        if l2 in l1 and f2 in f1:
            return 5
        
        # If the last note is in the last chord
        elif l2 in l1:
            return 3
    ...
    """
    Return the amount of steps where the note in the melody is not in the chord
    """
    def challenge(self, output, sequence):
        
        output_grid = progression_grid(output)
        sequence_grid = progression_grid(self.convert(sequence))
        
        differences = 0
        
        for s1, s2 in zip(output_grid, sequence_grid):
            if s2 not in s1:
                differences += 1
    ...
    def score(self, memory, sequence):
        ...
        chord_grid = rhythm_grid([length for _, length in memory.get('Chords')])
        ...
        chords_match = self.rhythm_overlap(chord_grid, sequence)        if self.cooperation else 0
        resolution = self.resolution(memory.get("Chords"), sequence)    if self.cooperation else 0
        challenge = self.challenge(memory.get("Chords"), sequence)      if self.cooperation else 0
        
        return continuity, energy, chords_match, resolution, challenge
    ...
    """
    Generate a base sequence with a progression that contains at least 3 notes
    """
    ...
    def base_sequence(self):
        ...
        metrics = continuity, energy, chords_match, resolution, challenge
    ...
    def convert(self, sequence):
        ...
        notes = [self.scale.note(note, 1) for note in progression]
        
        # Zip contents
        MIDI = list(zip(notes, rhythm))
